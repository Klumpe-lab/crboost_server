...
the per-TS strip all read from. No NiceGUI, no rendering — extracted from
``ui/tomo_dashboard_dialog.py`` (R0 refactor) so the strip/panel can consume a
clean data API. Verified by py_compile + ruff only (no runtime test in this
venv — see reference_hpc_env).
"""
