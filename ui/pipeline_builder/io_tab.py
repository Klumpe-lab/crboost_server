...
"""
I/O configuration tab: input slot overrides, output paths, readonly paths for frozen jobs.
"""
...
def render_io_tab(
    ...
    """Render the I/O configuration tab."""
    with ui.column().classes("w-full p-4"):
        if is_frozen:
            ...
            render_io_config(
                ...
                active_job_types=set(ui_mgr.selected_jobs),
            )
        ...
        for i, (key, value) in enumerate(paths_data.items()):
            ...
            with ui.row().classes(
                ...
                ui.label(snake_to_title(key)).classes(
                    "text-[10px] font-bold text-gray-500 uppercase w-32 shrink-0"
                )
                with ui.row().classes("flex-1 min-w-0 items-center gap-2"):
                    v_str = str(value)
                    ui.label(v_str).classes(
                        "text-xs font-mono text-gray-700 truncate flex-1 min-w-0"
                    ).tooltip(v_str)
                    ui.button(
                        icon="content_copy", on_click=lambda v=v_str: copy_path(v)
