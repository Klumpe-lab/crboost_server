...
class StatusPoller:
    ...
    async def check_and_update_statuses(self):
        ...
        sbatch_errors = panel.backend.pipeline_runner.get_sbatch_errors()
        if sbatch_errors:
            await panel.backend.pipeline_runner.stop_pipeline()
        ...
        all_done = (
            ...
            and panel.backend.pipeline_runner.active_schemer_process is None
        )
        if not all_done:
            return
    ...
    async def startup_sync(self):
        ...
        if not panel.ui_mgr.project_path:
            return
        await panel.backend.pipeline_runner.sync_all_jobs(str(panel.ui_mgr.project_path))
