...
*Grounded in the CryoBoost tutorial (the Copia recipe — Copia is 412's sister
retrotransposon capsid, same I1 workflow) and the PI's inner/outer-shell masking guidance.
...
flowchart TD
    ...
    MK["STEP 2 — build shell mask(s)<br/>the PI's inner/outer split"] --> R1
    A --> R1
...
2. **Low-pass the reference or you'll hallucinate.** Feed a refinement a sharp reference and it will
   ...
   low-passed — never the PI's high-res template — as the input.
...
| Symmetry | **I1** (keep this convention *everywhere* — it's what job002/003 + the PI's recipe used) |
...
**low-passed**; rely on gold-standard FSC; the PI's `try1` template is a *cross-check only*
(`/groups/klumpe/user/<pi>/Processing/412/try1/Class3D/job121/run_it015_class001.mrc`), never a
refinement input.
...
## 4. STEP 2 — Build the shell mask(s) *(the PI's inner/outer step)*
...
best pose. That's the PI's "so the alignment doesn't get confused by the other parts," verbatim.
...
This is the capsid-specific move and the PI's `94 × 60 × 13 ≈ 73k` arithmetic.
...
- [ ] **2c — ground truth:** what did **the PI's try1** reach on comparable 412 particles? (`/groups/klumpe/user/<pi>/Processing/412/try1/…` — NOT in Claude's mount; Artem checks.) the PI ≪ 35 Å at similar N → our data/extraction differs.
...
| 2c · PI try1 compare | data/extraction differs | — | ⬜ (Artem checks) |
...
**§12.5's "35.8 = honest particle-limited floor" is RETRACTED.** Hard counterexample from Artem: **the PI
(the PI) reached ~3× better resolution on a SMALLER 412 set.** So 94 particles is nowhere near the ceiling — a
...
1. **Tilt-series alignment / 3D-CTF fidelity (tomo-preprocessing) — top suspect for a 3× gap, NOT checkable from
   ...
   diff vs the PI's per-tilt geometry.**
...
**Decisive move:** get the PI's `try1` (`particles.star` / `tomograms.star` / refine command) — NOT mounted in
Claude's sandbox → Artem mounts or pastes. Diffing the *working* 412 pipeline against ours is the fastest route.
Order once the map is completed & eyeballed: maskless (#3) ∥ local (#2) → tomo-geometry / the PI diff (#1).

### 12.7 — PI try1 diff: ~35 Å is the SHARED 412 consensus ceiling, NOT our bug (2026-07-06, try1 mounted)

Diffed try1 directly. **The PI's converged gold-standard 412 consensus caps at the same wall we hit.**

| | Ours | PI try1 (best consensus, job135) |
| --- | --- | --- |
...
the §12.6 refine-flag suspects (#2 global search, #3 mask/`solvent_correct_fsc`) are RULED OUT: The PI varies
exactly those and hits the same wall.** 35 Å is the reproducible 412 consensus I1 gold-standard ceiling.

**The "the PI got 15 Å" red herring, killed:** 15.10 Å is `run_it001` ONLY = his `--ini_high 15` initial low-pass
...
that it1 low-pass as "current resolution" early in a run — the most likely origin of the "the PI got ~2–3× better"
impression.

**The PI has NO PostProcess / symmetry-expand / focused-capsomer / M in try1** — his best 412 result *is* the
...
consensus refine (it already matches the PI's). 412 is heterogeneous/flexible; whole-capsid consensus I1 averaging
...
| | 412 (ours) | 412 (PI try1) | **Copia (PI try2)** |
| --- | --- | --- | --- |
...
`refinedYTilt = −nominal + C` *exactly* (C = −8 ours, +15 PI-412, +9 Copia = a per-tomogram stage/axis offset).
Identical Warp→RELION convention everywhere; the sign-flip + constant offset is standard bookkeeping, not a
scramble. **Our tilt geometry is fine** (residuals live in the refined shifts, same as the PI/Copia).

**Why 412 caps ~3.5× worse than Copia, for everyone:** not flags (the PI varies them, same wall), not geometry
...
- **NOT particle quality (CC RETRACTION 2026-07-06).** Head-to-head LCCmax: **ours median 0.117 = Copia 0.117**,
  and BETTER than the PI's own 412 set (0.098). Copia hits 10 Å with the SAME CC we have; the PI hits 35 Å with
  WORSE CC. CC does NOT predict resolution → "our picks are noise" is WRONG (the NrSig/MaxProb I cited came from
  the 35 Å refine itself = circular). The PI's `job121` (K=1, 23 ptcl) is drawn from the SAME `FCIsolation`
  tilt-series we use and also lands ~35 Å ⇒ **35 Å is intrinsic to the 412 sample/data, not our picks.**
- **(C) a CryoBoost/412-COMMON processing issue** — ⚠️ NOT ruled out by "the PI also gets 35": he uses the SAME
...
- **412 heterogeneity (yellow flag)** — the PI ran **56 Class3D jobs** on 412 (try1) and still capped at 35; Copia
...
3. If a clean ~76 set still caps ~35 → 412 is heterogeneity-limited → 3D-classify for a rigid subpopulation
   first. (the PI's 56 Class3D jobs = he already fought exactly this.)
...
- ✅ ruled out: pick quality (CC ours=Copia), particle count (Copia 76→10), refine flags (the PI varies, same wall),
  tilt geometry (identical convention), CTF / defocus-hand / handedness (identical), gross tomo quality (tilts ~=).
- **LEADING: (A) 412 is an intrinsically harder / more heterogeneous sample.** Corroborated by the expert: The PI ran
...
The PI capping ~35 on the same tomos (can't classify out *continuous* size variation with 23–94 ptcl). Fair Copia
...
### 12.12 — Mask was too inclusive; the PI's outer-shell mask + the agreed plan (2026-07-06)

The PI flagged our solvent mask `sta_refine/mask_shell.mrc` as **too thick on the inside** (inner scoop
...
**AGREED PLAN (Artem + the PI, 2026-07-06):**
1. **Re-refine the consensus with the PI's outer-shell mask** — single-variable A/B vs our 23.85 Å. Script
   `sta_refine/run_refine3d_pimask.sh` (== `run_refine3d.sh` but `--solvent_mask` → his mask, `--o` →
   `sta_refine/Refine3D_pimask/`, everything else fixed) → `continue`/`--pad 1` combine → masked FSC number.
...
> **State (2026-07-07):** pimask re-refine DONE → **mask is not the lever** (14.31 Å = FSC artifact; the
> ~35 Å cap holds; full verdict §12.14). Gate: **capsomers DEFERRED behind §12.15** — the PI steered to squeeze the consensus first (Class3D-K1
...
> test) + `sta_refine/capsomer_2to6_RECIPE.sh` (fill geometry from the PI Q2 + a ChimeraX measure of the
...
- **6.2 Symmetry-expand** the outer-wall consensus poses:
  `relion_particle_symmetry_expand --i Refine3D_pimask/run_data.star --o symexpand/particles.star --sym I1`
...
### 12.14 — Mask A/B verdict + PI try1 combine: ~35 Å is the specimen; capsomers greenlit (2026-07-07)

**pimask re-refine (J2/J2b) DONE — RELION says 14.31 Å but it is a mask-inflation ARTIFACT, not a gain.**
- Refine 167290 converged it022 at `rlnCurrentResolution 35.77` (== baseline plateau), OOM'd on the pad-2
  final combine (identical death), then `continue_refine3d_pimask.sh` (177595, `--pad 1`) finished it →
  `Refine3D_pimask/run_class001.mrc`, reported **14.31 Å**.
- **Why it's not real** (`run_model.star` `rlnGoldStandardFsc`): FSC is high + continuous only to **35.8 Å**,
  then **cliffs to ~0 at 34.4 Å in BOTH runs** (baseline 0.882→0.003; pimask 0.878→0.000) — same cliff →
  property of the 94 particles, not the mask. RELION's "Final resolution" reports the *finest* shell where the
  sub-cliff noise last pokes above 0.143: baseline's blip = 23.85 Å (FSC 0.247), pimask's = 14.31 Å (FSC
...
- **Verdict:** the PI's outer-shell mask cleans the poses but does NOT break the cap (as predicted §12.12).
  **Read the FSC cliff (~35 Å), NOT the RELION "Final resolution" scalar, for this set.**

**The PI try1 combine also DIED — the ~35 Å agreement is now two independent refines.**
...
**Artefacts this session:** `sta_refine/for_pi_resolution.md` (blurb + questions for the PI),
`sta_refine/compare_pimask.cxc` (3 masks side-by-side), `sta_refine/summary.md` (J2/J2b + Finding + J3 stage).
...
geometry from the PI Q2 + a ChimeraX measure of the symexp'd map; 6.3 re-centre stays UNVERIFIED, Route B = M).
**[SUPERSEDED by §12.15: capsomers deferred — squeeze the consensus first.]**

### 12.15 — the PI's post-14Å steer: squeeze the consensus before capsomers (2026-07-07)

After reviewing the 14 Å run, the PI said **don't jump to symmetry expansion yet** — first hand auto-refine the
...
The PI's changes vs the pimask refine (§12.14):
1. **particle_diameter 550 → 600** — 550 was a bit tight for the capsid; make sure it fits.
2. **angular sampling → 1.8° = `--healpix_order 4`** (RELION GUI: order 4 = 1.8°; the pimask run started the
   global search at order 2 = 7.5° with a 1.8° local floor). ⚠️ **Interpretation flagged for the PI:** we set the
...
- **J2c** `class3d_k1_pimask.sh` — Class3D K=1, serial, his mask, diam 600, 1.8° → K=1 average + poses.
- **J2d** `run_refine3d_pimask_v2.sh` — auto-refine, MPI, seeded by J2c, same mask, diam 600, order 4 +
...
### 12.16 — J2c/J2d capped ~35 (4th time); the PI → C5 pentamer localized refine; the pipeline (2026-07-08)
...
34.4 Å** — same cliff as baseline/the PI. RELION's "12.4 Å (masked)" is a single sub-cliff noise blip → ignore.
So **the Class3D seed + diam 600 + order 4/5 changed nothing: 4th independent ~35 Å confirmation.** The
`compare_diam_sampling.cxc` half-map figure (filtered `run_it*_half1`, NOT `_unfil`) shows ours vs PI try1.

**OOM permanently fixed:** `run_refine3d_pimask_v2.sh` now runs `--pad 1` throughout (inert at ~35 Å, combine
fits 128G, one job, no A100 pin). Big-box high-res would need `--pad 2 --mem 256G --constraint=g4` instead.

**The PI's decision (2026-07-08):** proceed to **C5 pentamer** localized refinement. Working assumption: the
...
- **F1 — re-centre convention.** ✅ **AXIS RECOVERY PASSED (2026-07-08):** `verify_vertices.bild` = 12 markers in
  ...
  - **combo-2 ALSO = NOISE BALL (2026-07-08, job 206720). STOP RULE TRIGGERED** — no more blind flag-flipping.
    ...
    established tomo subboxing/localrec tool (**dynamo / M / scipion**) — the PI's wheelhouse, loop him. (3) Gut-check
    the PI: consensus disordered (weak icosahedral order) → pentamers worth the tool-up vs banking ~35 Å? →
...
- **F4 — C6 hexamers** (the 120, the bulk of the shell) after C5 + reconvene with the PI. → **DEFERRED**
...
knobs → chase; markers on mush → tell the PI before burning refine cycles).
...
convention) or **Scipion localrec** — loop the PI.
...
caps ~35 Å ⇒ honest specimen floor (bank it, loop the PI). Either way it's a number, not a blob.
...
### 12.19 — The mask was built wrong; two symmetry-based methods (the PI's placement + the colleague's carve); T=13 (2026-07-13)
...
the blobby average* — **thresholding a broken map**. The PI and a colleague both flagged this:
**the mask must come from a CLEAN SYMMETRIC object, produced by symmetry — not carved out of noise.**
Key reframe: our best pentamer already exists inside the I1 consensus (`Refine3D_pimask_v2/run_class001.mrc`,
...
**The PI's method (TRUSTED) — `sta_refine/pi_method/`.** `Klumpe-lab/analysis_tools :
...
`capsomer_2b` derived axes from RELION's own operators so it was I1 by construction. `pi_1_place_extract_validate.sh`
...
**The colleague's method — `sta_refine/colleague_method/`.** Carve one capsomer out of the clean I1
...
that frame — cleanest is to take the frame from the PI's placement and use the colleague's carve for the
shape/reference. Files: `colleague_1_carve_mask.cxc`, `colleague_2_soften_mask.sh`, `README.md`.

**Carve source for the colleague = `Refine3D_pimask_v2/run_class001.mrc`** (the clean I1 consensus). Both
folders are runnable in parallel; the PI's is the trusted path. `capsomer_9_mask.sh` (threshold-the-blob)
is **retired in spirit** — it only becomes valid fed the PI's clean `ref_c5/merged.mrc`.

**Kickoff — both investigations launched in parallel (2026-07-13).** Entry points:
- **The PI (trusted):** `sbatch sta_refine/pi_method/pi_1_place_extract_validate.sh` (place 1128 pentamers →
  extract box 128 → reconstruct C1 go/no-go + C5). Then, ONLY if `ref_c1_val/merged.mrc` is a clean centred
  flower, set `THR` and `sbatch pi_2_mask_and_refine_c5.sh` (mask from `ref_c5` + focused C5 local refine).
  `pi_2` is staged/gated: it fails fast unless `pi_1` ran and `THR` is set.
- **The colleague (parallel):** send the colleague the carve source map `Refine3D_pimask_v2/run_class001.mrc` + the
  scaffold `colleague_method/colleague_1_carve_mask.cxc`; he carves interactively (the 5 judgement calls in that
  file's header / `colleague_method/README.md`), then `colleague_2_soften_mask.sh`.
- **Success test for both:** the FSC cliff of a *clean 5-lobed pentamer* that beats `capsomer_8b`'s ~22 Å
  envelope. The PI's is the trusted mechanism; the colleague's is the independent cross-check on the mask shape.
- **Open risk still = the I1-vs-hardcoded-icosahedron convention**; `pi_1`'s C1 reconstruction is the test.
...
by baking a post-placement `awk` repair into `pi_1` (STAGE 1b) that restores the version line + a valid
`data_general` with the flag → structure now byte-matches the known-good `recenter_shiftonly/particles.star`.
Re-submit `pi_1`. (Report the round-trip bug upstream to the PI.)
...
directions), ORIENTATIONS ✗.** So the PI's geometry is right but his per-sub-particle orientation is wrong in
...
**loop the PI with the exact bug report** (in-chat). **Fallback that avoids his tool entirely:** we already have
...
capsomer_2b pentamers. Mush → 412 pentamer is too disordered; bank the ~24 Å consensus, loop the PI.
...
validated placement (capsomer_2b) produces a REAL central pentamer — decisively beats the PI's broken-tool
...
onto capsomer_2b poses, or hand-fix the PI's `R_combined` — are the swamp): **loop the PI** — one-line orientation
