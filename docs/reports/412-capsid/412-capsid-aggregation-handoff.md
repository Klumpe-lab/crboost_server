...
> capsomer → M), incl. the PI's inner/outer-shell masking. Gated on a Step-0 radial-profile measurement.
...
driving it toward the best-resolution average we can get. The PI made the
...
| PI's template (reference particle) | `/groups/klumpe/user/<pi>/Processing/412/try1/Class3D/job121/run_it015_class001.mrc` (448 px, 1.55 Å/px) |
| PI's reconstruct (recipe source) | `…/try1/Reconstruct/job118/` (`--b 384 --crop 224 --bin 2 --sym I1 --theme classic`) |
| PI's class3d (recipe source) | `…/try1/Class3D/job121/` (`--sym I1 --particle_diameter 550 --ini_high 45 --healpix_order 3 --offset 5/2 --K 1 --iter 15`) |
