# Roadmap 18 — Per-tilt CTF-fit outlier check: fail loudly when Warp's defocus fit diverges
...
stalling at 15 Å three jobs and two days later. Full investigation: `docs/copia-ctf-fit-investigation.md`.
...
| the awk scan in `docs/copia-ctf-fit-investigation.md` | the prototype of the 1D estimate: ±20-sample running-mean subtraction, correlation with CTF²(Δf), argmax over Δf; recovered 5.05–5.53 µm on all 28 copia tilts in every band while the 2D fit was off |
...
| `/groups/klumpe/user/sven.klumpe/Processing/Copia/try2` (1.55 Å/px, Warp dev31) | the 7 last-acquired tilts of Position_11_2 diverged (1.1–2.2 µm against ~4.8); below the fail fraction, warning only |
