...
checkpoint, `SmallSimpleCNN`, `model_state_dict` + `optimizer_state_dict`), next to the original author's
...
| Piece | State |
|---|---|
| `filterTilts/` vendored package (ImageProcessor, ModelLoader, model_architectures, PredictionThresholder, FilterStatistics + OOD check) | **in repo**, only cosmetically diverged from the original author's copy (ruff formatting) |
...
**From the original prototype, NOT worth porting:** the napari/Qt batch viewer (our gallery covers
...
the original author's tuning on old data → make them config, not constants, so real datasets can
...
- Manual overrides are already registry-stamped with probabilities — export (key, label, png)
  tuples as a retraining set for the model author; registry keys model versions so projects record
