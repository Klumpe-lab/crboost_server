...
species registry I get exactly one input field and that's it… it would be nice to have the
...
[roadmap_07-scoping-particle-registry-edges.md](../../picking_ui/roadmap_07-scoping-particle-registry-edges.md)); it is asked once per
species per project at first extract (D-3) and gets its own panel in
[roadmap_04-overview-declutter.md](roadmap_04-overview-declutter.md). Also not here: import-from-catalog — the rail
