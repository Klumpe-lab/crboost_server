...
Grounded in: `roadmap_04-stage0-census.md` §command-builder (18 builders, injection notes, "what ToolCommand
must support"), the §ASK decisions (2026-08-13), the byte-exact transcripts in
`roadmap_04-stage2-transcripts.md`, and a fresh read of `container_service.py` / `driver_base.py` /
...
4. **The parity oracle exists and is fleet-wide.** `run_command` now echoes
   ...
   commit = byte-identical `[run_command]` lines against `roadmap_04-stage2-transcripts.md` (§5).
...
- **Types with live transcripts (commits 1-5):** after migration, user re-runs the job type on
  ...
  full task coverage) and diffs the new `[run_command] $` lines against `roadmap_04-stage2-transcripts.md`.
...
- The old-March transcripts in the census stay useful as shape references only; the census itself
  flags the ts_ctf defocus-hand fragment as quoting-ambiguous in them. `roadmap_04-stage2-transcripts.md`
