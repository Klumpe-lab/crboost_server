...
> **DONE 2026-08-13** → `roadmap_04-stage0-census.md`: 75-entry divergence ledger (32 KEEP / 30 ALIGN /
...
> `roadmap_04-stage2-transcripts.md` (fsMotionAndCtf, tsImport, tsAlignment, tsCtf, tsReconstruct from
...
3. **Confirm the memory correction:** `denoise_predict` *is* array-migrated; `denoise_train` is
   ...
   adoption. (Confirmed against code 2026-08-13, see `roadmap_04-stage0-census.md`;
...
2. **`ToolCommand` + `run_tool()`** in `driver_base.py`: builder (`.arg()/.flag()/.paths()`) +
   ...
   > Design approved 2026-08-13 — `roadmap_04-stage2-toolcommand-design.md`, all five §7 decisions resolved
   ...
   > vs jobs run 16:15+). Every `[run_command]` line diffs clean against `roadmap_04-stage2-transcripts.md`
...
5. **Library relocations:** `drivers/subtomo_merge.py` → `services/` (then
   ...
   >
   > **#68 scheduled 2026-08-14** → `docs/roadmaps/completed/roadmap_07-extract-pick-list-job-identity.md`: full
