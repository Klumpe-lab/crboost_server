...
`docs/roadmaps/completed/roadmap_picks-filter-aggregation.md:196-199`), no `SingleFlight` on the open handler or async
...
- Wire `GateReport` / `extract_authoritative_pending` (`backend.py:368-380, ~529`) into the merge
  card as the pre-flight — this absorbs `docs/roadmaps/completed/roadmap_list-extraction-and-aggregation.md` §8.9 steps 4–7.
...
- Cleanup: `docs/roadmaps/completed/roadmap_picks-filter-aggregation.md:215,255` reference the already-deleted
...
- 2026-08-18 — **S6 CODE-COMPLETE**, both commits (`ruff check .` clean; `py_compile`,
  ...
  **Commit 2 — de-global + de-flag.**
  ...
  · **GateReport pre-flight wired** (absorbs `docs/roadmaps/completed/roadmap_list-extraction-and-aggregation.md` §8.9 steps 4-7):
  ...
  · Stale citations fixed in `docs/roadmaps/completed/roadmap_picks-filter-aggregation.md` (the deleted
  `merge_panel_component.py`, the moved modules, the moved merge button) and in
  `docs/roadmaps/completed/roadmap_list-extraction-and-aggregation.md`.
