...
`roadmap_04-tomo-import.md`.

## Registry interop — the "DataRegistry" question (D-10 context)

The TiltSeriesRegistry is mid-overhaul (`docs/roadmaps/roadmap_registry-consolidation.md`: Phase 1 backfill
...
> is `../roadmap_07-extract-pick-list-job-identity.md` (census #68 — per-list extraction becomes a real,
...
> `../roadmap_08-species-reactive-spine.md` (S0 fixes the "de-novo species not shown in the workbench" bug
...
|---|---|---|---|
| S1 | `roadmap_01-species-and-creation.md` | Species creation UX + registry hygiene + data-less project creation (kills `is_particle_only`) | — |
| S2 | `roadmap_02-geometry-and-inversion.md` | TomoGeometry provider, Particles-panel inversion, ArtiaX decoupled from CE dirs — full manual pick loop on imported tomos | S1 |
| S3 | `roadmap_03-extraction-and-resolver.md` | Candidate-free extraction + resolver wiring — chain reaches class3d | S2 |
| S4 | `roadmap_02-geometry-and-inversion.md` §S4 | Journey unification: delete fallback renderer, strip signature, empty-state copy, spinner failure paths | S2 |
| S5 | `roadmap_04-tomo-import.md` | Import hardening: producer sentinel, multi-import, formats, perf, runtime shake | — (reads benefit from S2) |
| S6 | `roadmap_05-aggregation-module.md` | Aggregation factor-out + de-global + `is_aggregation` removal | — |
| 07 | `../roadmap_07-extract-pick-list-job-identity.md` | extract_pick_list job identity + per-list status UI (census #68; closes #73) | S3 (spine landed) |
...
(`roadmap_04-tomo-import.md`); P3 (Aggregate provider job) → dropped — the merge card moves to the
PARTICLES header in S6 instead; P4 per-slot acquire UX → out of scope here (revisit after S6);
P5 (`is_aggregation` removal) → S6. `docs/roadmaps/completed/roadmap_list-extraction-and-aggregation.md` stays as reference;
its §8.9 steps 4–7 are absorbed by `roadmap_05-aggregation-module.md`.
