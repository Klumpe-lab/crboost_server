...
| 11 | [picks-actions-consolidation](11-picks-actions-consolidation.md) | `ListRef` + `list_actions` carve; Picks tab (cross-tomo table, extract-all-pending via GateReport); Journey declutter (Journey = look & curate + ⚡; Species = manage & act); Curation tab v1 | medium (Journey carve, parity-gated) | **S1 + S2 code-complete** — S1 2026-08-16 (`services/particles/list_ref.py` + `ui/particles/list_actions.py`, Journey repointed, `deduplicate_pick_list` list-keyed), S2 2026-08-17 (`ui/species/picks_tab.py`: cross-tomo table on `species_overview`, per-row extract / dedup / delete, per-tomo merge bar + auth radio, "Extract all pending" behind the gate-report pre-flight, `journey_show_ts` hook, `list_actions.dialog_host()`); BOTH PENDING RUNTIME (S1 parity walkthrough + S2 §2 checklist); S3 (declutter) + S4 (Curation tab) next; live extraction status still gated on 07 |
