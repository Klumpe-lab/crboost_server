...
| 04 | [driver-consolidation](04-driver-consolidation.md) | ArrayDriver template, ToolCommand, one status protocol | medium (behavior-preserving by construction, per-driver rollout) | **stages 0–2 done** — census + `TaskStatusStore` + all 14 `ToolCommand`/`run_tool` migrations, the 5 transcript-covered types byte-verified on a live run 2026-08-13 ([design](04-stage2-toolcommand-design.md), [transcripts](04-stage2-transcripts.md)). Stage 3: `ArrayDriver` + `DriverContext` + ts_reconstruct + ts_ctf done, 6 drivers left. Stages 4–5 deferred by the maintainer |
...
| — | [denovo_picking/](denovo_picking/00-overview.md) | De-novo species + manual picking; tomo-import hardening; project-type dissolution; aggregation factor-out | medium (dashboard inversion, parity-gated) | **S1 + S2 code-complete** 2026-08-13 (species creation + registry hygiene; `TomoGeometry` provider, Particles-panel inversion, ArtiaX decoupled from candidate-extract dirs) — runtime owed on both; S3 next. Decisions D-1/D-7/D-10 locked; supersedes `notes/PARTICLE_PROJECT_ROADMAP.md` |
