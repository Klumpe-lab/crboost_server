...
| 10 | [species-page](10-species-page.md) | "Species" page replaces the Template Workbench view: rail + compact tabs (Overview · Templates & masks [mounted workbench] · Picks · Curation · Jobs); identity editor + delete cascade → service; Jobs tab with drift chips | medium | **S1+S2 (one commit) + S3 code-complete** 2026-08-16 (shell + rail + segmented tabs + `create_species` + mounted workbench tab; Overview = identity editor (P-02 closed) + status/sanity block + delete → `services/species_admin`; PENDING RUNTIME) |
