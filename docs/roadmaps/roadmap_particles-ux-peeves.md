...
written by the maintainer in the maintainer's walkthrough notes (not in the repo);
...
| P-12 | Job tabs (TM) | the boxed "Template, mask & search symmetry" card clashes with the parameter form; needs a guideline for every species-derived field | cosmetic (guideline) | picking_ui/02 + `completed/picking_ui/roadmap_00-overview.md` §guideline | code-complete 2026-08-18, PENDING RUNTIME |
...
| P-49 | Job roster / queue | a just-added job paints the same amber "Scheduled" dot as one queued on the cluster — the roster cannot show staged-but-not-yet-queued | structural (semantic) | [13-roster-staged-vs-queued](roadmap_13-roster-staged-vs-queued.md) | scoped 2026-08-23 |
...
| P-56 | Job roster | deleted jobs come back on server restart — `remove_instance_from_pipeline` mutated `state.jobs` then called `save_project()` WITHOUT force, and nothing on that path marks the state dirty (`ensure_job_initialized` only calls `update_modified()`), so `StateService.save_project`'s `force or is_dirty` gate skipped the write every time | bug | fixed 2026-08-23 (`mark_dirty()` + `force=True`, explicit project path, `pipeline_order` pruned) | fixed, PENDING RUNTIME |
| P-57 | Job roster | both delete confirmations were parented in a roster row that the 3 s FingerprintedView poll rebuilds — a tick landing while the dialog is open destroys it and the delete silently never happens | bug (latent) | fixed 2026-08-23 (`dialog_host()`) | fixed, PENDING RUNTIME |

Add rows as the walkthrough produces them; keep the peeve text short and put the long form in
the maintainer's walkthrough notes.
...
- P-12 species-derived fields follow the guideline in `completed/picking_ui/roadmap_00-overview.md` → **picking_ui/02**

### Roster / queue
- P-49 staged vs queued (the roster claims work is in flight before Run) → [13-roster-staged-vs-queued](roadmap_13-roster-staged-vs-queued.md)
...
- 2026-08-17 — still ONE raw peeve intake so far. The walkthrough (Journey / Species / roster →
  the maintainer's walkthrough notes) has not happened; the cosmetic checklists stay near-empty until it
...
- 2026-08-18 (later, same session) — **P-10…P-24 are code-complete**: picking_ui 01–06 were all built
  ...
  maintainer has run the jobs, and the three items in the maintainer's notes (beam-induced-motion
...
- 2026-08-21 — **the promised second intake happened** (Picks/Curation tabs, ChimeraX control,
  gallery rework, chrome round 2 — the maintainer's notes). P-32…P-48 added; homed in the
  ...
  Still deliberately uncollected (maintainer's "don't touch this yet"):
