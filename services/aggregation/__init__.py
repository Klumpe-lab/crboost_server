...
- ``extraction`` — per-list subtomo-extraction inputs (schema source, list star, geometry)
  ...
  one mutator, which is INVOCATION-scoped (the merge card's actions call it) and not
  render-scoped. It was ``authoritative`` until the authoritative-list model was removed —
  what feeds a refinement is the source set the user selects in the merge card, not a stored
  per-tomogram nomination.
