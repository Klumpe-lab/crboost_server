...
class TemplateService:
    ...
    async def generate_basic_shape_async(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        # We use run_in_executor to ensure logging prints immediately to stdout
        return await asyncio.get_running_loop().run_in_executor(
            None, 
            lambda: self._generate_basic_shape_sync(*args, **kwargs)
        )
    ...
    # --- Synchronous Implementations (Run in Threads) ---

    def _fetch_emdb_map_sync(self, emdb_id: str, output_folder: str) -> Dict[str, Any]:
        try:
            emdb_id = emdb_id.strip().replace("EMD-", "")
            if not emdb_id: 
    ...
    def _process_volume_sync(self, input_path: str, output_folder: str, target_apix: float, target_box: int, tag: str = "") -> Dict[str, Any]:
    ...
    def _crop_or_pad(self, data, size):
        ...
        if np.array_equal(curr, target): return data
        ...
        new_data[
            start_targ[0]:end_targ[0], 
            start_targ[1]:end_targ[1], 
            start_targ[2]:end_targ[2]
        ] = data[
            start_curr[0]:end_curr[0], 
            start_curr[1]:end_curr[1], 
            start_curr[2]:end_curr[2]
        ]
        return new_data
    ...
    async def create_mask_relion(self, input_vol, output_mask, threshold, extend, soft, lowpass):
        ...
        try:
            ...
            cmd = f"relion_mask_create --i {abs_in} --o {abs_out} --ini_threshold {threshold} " \
            ...
            res = await self.backend.run_shell_command(
                cmd, 
                cwd=Path(os.path.dirname(abs_out)), 
                tool_name="relion", 
