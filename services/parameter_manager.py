...
class ParameterManager:
    ...
    def update_parameter(self, param_path: str, value: Any):
        ...
        try:
            ...
            if param_path.startswith("jobs."):
                ...
                if len(parts) != 3:
                    raise ValueError(
                        f"Invalid job parameter path: {param_path}. Expected format: jobs.<job_name>.<field_name>"
                    )
                ...
                if hasattr(job_obj, field_name):
                    ...
                    print(
                        f"[PARAMS DIRECT] ✓ Updated {job_name}.{field_name} = {value}"
                    )
                else:
                    available_fields = list(job_obj.dict().keys())
                    raise ValueError(
                        f"Field '{field_name}' not found in {job_name}. Available: {available_fields}"
                    )
            ...
            # Handle global parameters (e.g., "microscope.voltage")
                ...
                if hasattr(category_obj, field_name):
                    ...
                    self.state.update_modified()
                    print(
                        f"[PARAMS DIRECT] ✓ Updated {category}.{field_name} = {value}"
                    )
                else:
                    available_fields = list(category_obj.dict().keys())
                    raise ValueError(
                        f"Field '{field_name}' not found in {category}. Available: {available_fields}"
                    )

            else:
                raise ValueError(
                    f"Unsupported parameter path: {param_path}. Must start with 'jobs.', 'microscope.', 'acquisition.', or 'computing.'"
                )
    ...
    def update_from_mdoc(self, mdocs_glob: str):
        ...
        try:
            ...
            if "image_size" in mdoc_data:
                ...
                if len(dims) == 2:
                    self.state.acquisition.detector_dimensions = (
                        int(dims[0]),
                        int(dims[1]),
                    )
                    ...
                    if (
                        ...
                        print("[PARAMS-V2] Detected K3/EER camera, set fractions to 32")
    ...
    def prepare_job_params(
        self, job_name: str, job_star_path: Optional[Path] = None
    ) -> BaseModel:
    ...
    def export_for_project(
        ...
        print(
            f"[PARAMS-V2 DEBUG] Exporting project with microscope.pixel_size_angstrom = {self.state.microscope.pixel_size_angstrom}"
        )
        ...
        for job in selected_jobs:
            ...
            if job not in self.state.jobs:
                ...
                self.state.populate_job(
                    job, template_path if template_path.exists() else None
                )
            else:
                print(
                    f"[PARAMS-V2 DEBUG] Job {job} already exists, pixel_size = {getattr(self.state.jobs[job], 'pixel_size', 'N/A')}"
                )
    ...
    def save_to_file(self, path: Path):
        ...
        try:
            state_dict = {
                ...
                "computing": self.state.computing.dict(),
                "jobs": {
                    name: params.dict() for name, params in self.state.jobs.items()
                },
                "metadata": {
    ...
    def get_ui_state(self) -> Dict[str, Any]:
        ...
        ui_state = {
            ...
            "pixel_size_angstrom": {
                "value": self.state.microscope.pixel_size_angstrom,
                "source": "user",
            },
            "acceleration_voltage_kv": {
                "value": self.state.microscope.acceleration_voltage_kv,
                "source": "user",
            },
            "spherical_aberration_mm": {
                "value": self.state.microscope.spherical_aberration_mm,
                "source": "user",
            },
            "amplitude_contrast": {
                "value": self.state.microscope.amplitude_contrast,
                "source": "user",
            },
            "dose_per_tilt": {
                "value": self.state.acquisition.dose_per_tilt,
                "source": "user",
            },
            "detector_dimensions": {
                "value": self.state.acquisition.detector_dimensions,
                "source": "user",
            },
            "tilt_axis_degrees": {
                "value": self.state.acquisition.tilt_axis_degrees,
                "source": "user",
            },
            "eer_fractions_per_frame": {
                "value": self.state.acquisition.eer_fractions_per_frame,
                "source": "user",
            }
