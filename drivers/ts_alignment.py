...
def build_alignment_commands(params: TsAlignmentParams, paths: dict[str, Path], num_tomograms: int) -> str:
    ...
    cmd_parts_import = [
        "WarpTools ts_import",
        "--mdocs", mdoc_dir,
        "--pattern", shlex.quote(params.mdoc_pattern),  # Use configurable pattern
    ...
    if params.alignment_method == AlignmentMethod.ARETOMO:
        cmd_parts_align = [
            ...
            "--alignz", str(int(params.thickness_nm * 10)),  # Convert nm to Å
        ...
        cmd_parts_align = [
            ...
            "--settings", settings_file,
            "--angpix", str(params.rescale_angpixs),
            "--patch_size", str(int(params.imod_patch_size * 10)),  # Convert nm to Å
        ]
    else:
