...
def main():
    ...
    try:
        ...
        for i, command in enumerate([check_hand_command, set_hand_command, ctf_command], 1):
            ...
            result = container_service.execute_command(
                command=command,
                cwd=job_dir,
                tool_name=params.get_tool_name(),
                additional_binds=[str(project_root)]
            )
            
            if not result["success"]:
                raise Exception(f"Command {i} failed: {result.get('error', 'Unknown error')}")
